...
function NoAccount(props: { addAccount: () => void, refreshList: () => void }) {
	...
	return (<>
		<div className='flex flex-col gap-4'>
			...
			{ import.meta.env.VITE_DEMOMODE === "true" ? <>
				<div className='flex bg-white dark:bg-slate-800 p-2 gap-4 rounded-md shadow-sm items-center opacity-50 cursor-not-allowed'>
					<div>
						<Icon path={mdiQrcodeScan} size={1.5} />
					</div>
					<div>
						<div className='text-xl'>{t('HOME_EMPTYSCREEN_SYNC_TITLE')}</div>
						<div className='text-slate-500'>{t('HOME_EMPTYSCREEN_SYNC_NOTAVAILABLE_DESC')}</div>
					</div>
				</div>
